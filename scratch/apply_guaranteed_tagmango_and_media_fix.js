...
// ==============================================================
// 1. UPDATE SERVER.JS: SYNCHRONOUS TAGMANGO API CREDIT & LOGGING
// ==============================================================
let serverCode = fs.readFileSync('server.js', 'utf8');

const sSubMarker = "app.post(['/api/submissions', '/gamification/api/submissions'], async (req, res) => {";
const eSubMarker = "res.json({ success: true, message: 'Submission saved and LCs credited to TagMango wallet', data: newSub });";

const sIdx = serverCode.indexOf(sSubMarker);
const eIdx = serverCode.indexOf(eSubMarker);
...
if (sIdx !== -1 && eIdx !== -1) {
    ...
    serverCode = serverCode.substring(0, sIdx) + newServerSubBlock + '\n\n' + serverCode.substring(eIdx + eSubMarker.length);
    fs.writeFileSync('server.js', serverCode, 'utf8');
    console.log('Successfully updated server.js with synchronous TagMango points assignment and error handling.');
}

// ==============================================================
// 2. UPDATE APP.JS: SYNCHRONOUS LOCAL SAVE, INSTANT CARD TRANSITION,
// FULL PLAYABLE VIDEO & AUDIO REVIEW MODALS
// ==============================================================
let appCode = fs.readFileSync('app.js', 'utf8');

// Global media store in app.js
const mediaHelpersCode = `
// ==============================================================
...
if (!appCode.includes('window._recordedVideoData')) {
    const sPos = appCode.indexOf('var _audioStream = null;');
    if (sPos !== -1) {
        appCode = appCode.substring(0, sPos) + mediaHelpersCode + '\n' + appCode.substring(sPos);
    }
}

// Update startAudioRecording & startVideoRecording to cache data immediately
const audioStopOld = `_audioRecorder.onstop = () => {
...
const audioStopNew = `_audioRecorder.onstop = () => {
            const blob = new Blob(_audioChunks, { type: 'audio/webm' });
...
if (appCode.includes(audioStopOld)) {
    appCode = appCode.replace(audioStopOld, audioStopNew);
}

const videoStopOld = `_videoRecorder.onstop = () => {
...
const videoStopNew = `_videoRecorder.onstop = () => {
            const blob = new Blob(_videoChunks, { type: 'video/webm' });
            const blobUrl = URL.createObjectURL(blob);
            window._recordedVideoData = window._recordedVideoData || {};
...
if (appCode.includes(videoStopOld)) {
    appCode = appCode.replace(videoStopOld, videoStopNew);
}

// Update submitCheckinForm to save IMMEDIATELY upon clicking Submit
const submitOldStart = 'async function submitCheckinForm(dayNum, moduleName, cardDateKey, lcOnTime, lcLate, endTime) {';
const submitOldEnd = 'window.submitCheckinForm = submitCheckinForm;';

const subS = appCode.indexOf(submitOldStart);
const subE = appCode.indexOf(submitOldEnd);

if (subS !== -1 && subE !== -1) {
    const perfectSubmitBlock = `async function submitCheckinForm(dayNum, moduleName, cardDateKey, lcOnTime, lcLate, endTime) {
    ...
    // 1. SAVE IMMEDIATELY TO LOCAL STORAGE DB (Synchronous - Instant completion)
    ...
    // 2. CREDIT USER BALANCE IMMEDIATELY
    if (currentUser) {
    ...
    // 3. POST TO SERVER IMMEDIATELY (Async dispatch to TagMango Wallet API)
    apiFetch('/api/submissions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(subData)
    }).then(r => r.json()).then(data => {
        console.log('✅ Server Submission & TagMango Wallet Response:', data);
    }).catch(e => console.error('Submission sync error:', e));

    // 4. Close submission form modal
    document.getElementById('submissionModalDynamic')?.remove();

    // 5. Show AI evaluating progress modal with realistic lagtime (18 seconds with 5 stages)
    showAiEvaluatingLagtime(pointsAwarded, () => {
        // Update UI immediately upon completion
    ...
    // Also update timeline card immediately in background so it never shows "Start check-in"
    if (typeof switchMilestoneTab === 'function') {
        switchMilestoneTab(moduleName);
    }
}`;
    appCode = appCode.substring(0, subS) + perfectSubmitBlock + '\n' + appCode.substring(subE + submitOldEnd.length);
}

// Update renderSubmissionDetailModal to display guaranteed video player and audio player
const rModalStart = 'function renderSubmissionDetailModal(sub, userId, dayLabel, type) {';
const rModalEnd = 'window.renderSubmissionDetailModal = renderSubmissionDetailModal;';

const rS = appCode.indexOf(rModalStart);
const rE = appCode.indexOf(rModalEnd);

if (rS !== -1 && rE !== -1) {
    const perfectReviewBlock = `function renderSubmissionDetailModal(sub, userId, dayLabel, type) {
        ...
        bodyHtml = \`
            <div class="space-y-4">
                \${responses.map((r, i) => {
                    ...
                    let rawMedia = r.videoUrl || r.audioUrl || r.fileData || r.answer || r.value || '';
                    let isAudio = (qType === 'audio') || String(qTitle).toLowerCase().includes('audio') || String(qTitle).toLowerCase().includes('voice') || String(qTitle).toLowerCase().includes('mic');
                    let isVideo = (qType === 'video') || String(qTitle).toLowerCase().includes('video') || String(qTitle).toLowerCase().includes('camera');
    ...
    appCode = appCode.substring(0, rS) + perfectReviewBlock + '\n' + appCode.substring(rE + rModalEnd.length);
}

fs.writeFileSync('app.js', appCode, 'utf8');
console.log('Successfully updated app.js with instant synchronous submission saving and guaranteed video/audio player.');
